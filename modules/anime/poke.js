...
exports.run = async (client, message, args, settings) => {
    ...
    try {
    let results = await fetch(`https://nekos.life/api/v2/img/poke`)
    .then(res => res.json())
    .then(json => link = (json.url))

    let embed = new Discord.MessageEmbed()
    .setColor("#f2f8eb")
    .setTitle((message.author.username) + " Poked " + (user.username))
...
exports.data = {
  name: "poke", // name of command used on lists etc...
  category: "Anime", // what category the command is in
  usage: "poke (`@username`)", // how to use the command
  description: "lets you poke a user!", // description of what the command does
